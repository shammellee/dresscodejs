...
(function(root, factory) {
  ...
  var _style, _stylesheet, _lookup, _variable_exists, Dresscode;
  ...
  Dresscode = {
    ...
    init: function init() {
      ...
      (function() {
        ...
        if (
          (_refresh_interval = _lookup.call(
            Dresscode,
            'refreshInterval',
            Number
          ))
          ...
          _meta.content = _refresh_interval < 1 ? 1 : _refresh_interval;
    ...
    dress: function dress() {
      var _nodes = Array.prototype.slice.call(
          document.querySelectorAll('[data-dresscode]')
        ),
        _ruleset,
        _rule,
        _keys,
      ...
      for (i = 0; i < _nodes.length; i++) {
        _ruleset = new this.RuleSet(_nodes[i]);

        for (
          _keys = Object.keys(_ruleset.rules), k = 0;
          k < _keys.length;
          k++
        ) {
          _key = _keys[k];
  ...
  Dresscode.RuleSet.prototype = {
    lookup: function lookup(_variable_key, _default) {
      _variable_key = _variable_key.replace(
        Dresscode.regex.KEBAB_CASE,
        Dresscode.helpers.to_camel_case
      );
      _default = _default || null;
    ...
    parse_value: function parse_value(_rule) {
      ...
      _result = _result.replace(_percent, '$1%').replace(_decimal, '$1.$2');
